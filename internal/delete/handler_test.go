...
func (suite *DeleteHandlerTestSuite) Test_ESReturnsNoResults() {
	...
	suite.Contains(suite.RespBody(), `"errors":[{"name":"request","description":"could not find document to delete"}]`)
}
...
func (suite *DeleteHandlerTestSuite) Test_ESReturnsMultipleResults() {
	...
	suite.Equal(http.StatusInternalServerError, suite.RespCode())
	suite.Contains(suite.RespBody(), `"errors":[{"name":"request","description":"deleted 2 documents matching query"}]`)
}
